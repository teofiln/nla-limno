...
# summary with urban as group
thermo |>
  group_by(urban) |>
  summarise(
    mean_value = mean(thermocline_depth, na.rm = TRUE),
    median_value = median(thermocline_depth, na.rm = TRUE),
    n_lakes = n()
  )
...
d7 |>
  group_by(urban) |>
  summarise(
    mean_value = mean(secchi, na.rm = TRUE),
    median_value = median(secchi, na.rm = TRUE),
    n_lakes = n()
  )

plot_variable_by_group <- function(data, variable, group, binwidth = 1) {
  ggplot(data, aes(x = .data[[variable]])) +
    geom_histogram(binwidth = binwidth, fill = "white", color = "steelblue") +
    facet_wrap(~ .data[[group]]) +
    labs(
      title = paste("Distribution of", variable, "by", group),
      x = paste(variable, "(units)"),
      y = "Count of Lakes"
    ) +
    theme_minimal()
...
plot_variable_by_group(d7, "secchi", "urban", 0.5)
plot_variable_by_group(d7, "do", "thermocline_depth", 0.5)
